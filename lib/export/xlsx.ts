...
export interface ExportPayload {
  ...
  /** Overall AI thesis/rationale commentary (rendered in full in the Summary sheet). */
  thesis?: string | null;
 ...
 * Two sheets: "Summary" (title/subtitle/full thesis) and "Allocation" (the table).
 */
export async function buildExportWorkbook(payload: ExportPayload): Promise<Buffer> {
  ...
  if (payload.thesis) {
    ...
    summary.mergeCells(`A${row}:C${row}`);
    // Size the row to the thesis length so the FULL rationale stays visible
    // (instead of a fixed 80pt clip that hides long theses).
    const approxCharsPerLine = 86; // Summary spans columns A:C (24 + 40 + 24 ≈ 88)
    const thesisLines = Math.max(1, Math.ceil(payload.thesis.length / approxCharsPerLine));
    summary.getRow(row).height = Math.min(400, thesisLines * 15 + 10);
    row += 2;
  }
