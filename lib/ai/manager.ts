...
const PORTFOLIO_BUILD_PATTERNS = [
  ...
  // between the (now-optional) article and "portfolio" (e.g. "build me a $2k
  // healthcare portfolio", or "build me portfolio for my cash" with no article).
  ...
  // silently defaulted to individual stocks). The article "a/an/some" is
  // OPTIONAL so "build portfolio" / "build me portfolio" still route to the
  // vehicle triage instead of being silently defaulted by the model.
