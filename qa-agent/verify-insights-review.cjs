...
(async () => {
  ...
  console.log('\n=== B. concentration rail: ring centred, tight 3-line legend ===');
  {
    const { ctx, page } = await setup(browser, { dsf: 3 });
    if (await gotoInsights(page)) {
      const r = await readRail(page);
      if (r.found) {
        rec('B1 rail is a fixed 108px column', Math.abs(r.railW - 108) <= 0.6, `${r.railW}px`);
        ...
        rec('B5 ring→legend gap tight (≤8px, CSS says 6)', r.ringToLegend <= 8 && r.colGap === 6, `gap=${r.ringToLegend} cssGap=${r.colGap}`);
        rec('B6 legend rows tight (CSS row gap 4)', r.rowGapCss === 4, `rowGap=${r.rowGapCss} measured=${r.rowGaps.join('/')}`);
        rec('B7 exactly 3 legend rows (top-2 + Other, real data)',
          r.rows.length === 3 && r.rows.map((x) => x.text).join(' | ') === expRows.join(' | '),
  ...
  console.log('\n=== C. Ask Rufus bar vs content (reserved space) ===');
  {
    ...
    if (await gotoInsights(page)) {
      ...
      rec('C3 reserved padding behind the bar ≥ bar footprint', bottom.scrollerPadBottom !== null && bottom.barHeight !== null && bottom.scrollerPadBottom >= bottom.barHeight,
        `padBottom=${bottom.scrollerPadBottom} barHeight=${bottom.barHeight}`);
