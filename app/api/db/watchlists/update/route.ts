...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const authUserId = authUser!.id;
    ...
    const { watchlistId, name, description } = body as { watchlistId?: string; name?: string; description?: string };
    if (!watchlistId) return NextResponse.json({ error: 'watchlistId required' }, { status: 400 });
    if (!name?.trim()) return NextResponse.json({ error: 'name required' }, { status: 400 });

    const { data: existing } = await (supabase as any)
      .from('watchlists').select('id, user_id').eq('id', watchlistId).maybeSingle();
