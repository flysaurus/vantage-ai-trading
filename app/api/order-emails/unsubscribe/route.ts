/**
 * GET /api/order-emails/unsubscribe
 *
 * One-click unsubscribe for transactional order emails (placed/filled/cancelled).
 * No login required. Token = HMAC-signed userId (same scheme as the Portfolio
 * Agent digest), verified server-side via verifyUnsubscribeToken().
 *
 * Sets order_emails_enabled = false on the user's record, then renders a
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const { error } = await supabase
    .from('users')
    .update({ order_emails_enabled: false })
    .eq('id', userId);

  if (error) {
    console.error(`[order-unsubscribe] Failed to update user ${userId.slice(0, 8)}:`, error.message);
  ...
  console.log(`[order-unsubscribe] User ${userId.slice(0, 8)} unsubscribed from order emails`);
...
<head>
  ...
  <title>Vantage — Order Email Preferences</title>
</head>
<body style="margin: 0; padding: 0; background: #0a0f1a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #e2e8f0; display: flex; align-items: center; justify-content: center; min-height: 100vh;">
  <div style="text-align: center; max-width: 400px; padding: 40px 24px;">
    ...
    <div style="font-size: 14px; color: #94a3b8; line-height: 1.6; margin-bottom: 24px;">
      ${success
        ? "You won't receive any more order notification emails (placed / filled / cancelled). You can re-enable them anytime in the Vantage app settings."
